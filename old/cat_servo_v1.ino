 ...
 * PIR_PIN     -> D2 (GPIO4)   - PIR output
 * LED_PIN     -> D4 (GPIO2)   - Status LED (with ~320Ω to GND)
 * MODE_A_PIN  -> D6 (GPIO12)  - 3-pos switch pole A (common)
 * MODE_B_PIN  -> D7 (GPIO13)  - 3-pos switch pole B (common)
 */

// ===== MASTER TOGGLES =====
const bool USE_PIR    = false;  // true = PIR-gated sessions
...
// ===== Startup warmup (PIR OFF mode) =====
const unsigned long STARTUP_WARMUP_MS = 5000UL;

// PIR behavior (only used when USE_PIR=true)
const unsigned long PIR_RETRIGGER_COOLDOWN_MS = 30000UL;

// ===== AM312 handling =====
const unsigned long PIR_WARMUP_IGNORE_MS   = 15000UL;
const unsigned long PIR_EDGE_DEBOUNCE_MS   = 250UL;
const unsigned long PIR_STUCK_HIGH_MS      = 20000UL;
const unsigned long PIR_LOW_CLEAR_MS       = 500UL;
...
bool pirSuppressed = false;

// ===== Auto-cycle (when USE_PIR == false) =====
...
// ---- PIR state resync ----
void resyncPirState(unsigned long now) {
  int cur = digitalRead(PIR_PIN);
  lastPirState = cur;
  lastPirEdgeMs = now;

  pirStuckHigh = false;
  pirHighSinceMs = (cur == HIGH) ? now : 0;
  pirLowSinceMs  = (cur == LOW)  ? now : 0;
}

// ===== PIR: rising edge + stuck-high guard =====
bool pirRisingEdge(unsigned long now) {
  if (!pirReady) return false;
  if (pirSuppressed) return false;
  ...
  if (cur == HIGH) {
    ...
    pirLowSinceMs = 0;
    if (!pirStuckHigh && (now - pirHighSinceMs >= PIR_STUCK_HIGH_MS)) pirStuckHigh = true;
  } else {
    pirHighSinceMs = 0;
    if (pirLowSinceMs == 0) pirLowSinceMs = now;

    if (pirStuckHigh && (now - pirLowSinceMs >= PIR_LOW_CLEAR_MS)) {
      pirStuckHigh = false;
      lastPirEdgeMs = now;
      lastPirState = LOW;
    }
  ...
  bool rising = (cur == HIGH && lastPirState == LOW);
  lastPirState = cur;

  if (pirStuckHigh) return false;

  if (rising) {
    if (now - lastPirEdgeMs < PIR_EDGE_DEBOUNCE_MS) return false;
    lastPirEdgeMs = now;
    return true;
  }
  return false;
...
// ===== LED rhythms (visible + distinguishable) =====
void blinkLedDuringSession(PlayMode mode) {
  ...
  if (TEST_MODE) {
    ...
    switch (mode) {
      case MODE_LAZY:    offMs = 120; onMs = 120; break;
      case MODE_PLAYFUL: offMs = 80;  onMs = 80;  break;
      case MODE_ZOOMIES: offMs = 50;  onMs = 50;  break;
    }
  }
...
int dartSpeedForMode(PlayMode mode) {
  ...
  switch (mode) {
    case MODE_LAZY:    return 6;  // was 9
    case MODE_PLAYFUL: return 3;  // was 4
    case MODE_ZOOMIES: return 2;  // was 3
  }
  return 3;
}
...
// Darts: big % of safe range (lets you use more angle without clipping)
...
void runSessionForMode(PlayMode mode) {
  ...
  bool allowMidChaos = (!USE_PIR) && (((sessionCounter + 1UL) % 3UL) == 0UL);
  ...
  resyncPirState(lastSessionEndMs);
  pirSuppressed = false;
...
void setup() {
  ...
  // Auto-center REST to maximize usable range (prevents “clipping” one side)
  if (AUTO_CENTER_REST) {
...
void loop() {
  ...
  // ===== Startup warmup (only meaningful when PIR is OFF) =====
  if (!USE_PIR && now < startupEndMs) {
    if (currentServoDeg != SERVO_REST_DEG) moveTo(SERVO_REST_DEG, 10);
    startupHeartbeat(now);
    delay(10);
    return;
  } else if (!USE_PIR && !firstSessionHasRun) {
    digitalWrite(LED_PIN, HIGH);
  }

  // ===== PIR warm-up (only when PIR is enabled) =====
  if (USE_PIR && !pirReady) {
    if (now - bootMs >= PIR_WARMUP_IGNORE_MS) {
      pirReady = true;
      resyncPirState(now);
    } else {
      delay(20);
  ...
  if (USE_PIR) {
    bool cooldownOver = (lastSessionEndMs == 0) || ((now - lastSessionEndMs) > PIR_RETRIGGER_COOLDOWN_MS);
    if (!sessionActive && cooldownOver && pirRisingEdge(now)) shouldStart = true;
  } else {
    // Macro run/nap cycle
  ...
  if (shouldStart) {
    ...
    if (!USE_PIR) firstSessionHasRun = true;
  }
